...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ActiLinkTheme {
                val (currentUser, setCurrentUser) = remember { mutableStateOf(auth.currentUser) }
                var selectedScreen by remember { mutableStateOf(if (currentUser != null) Screen.Home else Screen.Auth) }

                LaunchedEffect(auth) {
                    auth.addAuthStateListener { firebaseAuth ->
                        setCurrentUser(firebaseAuth.currentUser)
                        selectedScreen = if (firebaseAuth.currentUser != null) Screen.Home else Screen.Auth
                    }
                }
